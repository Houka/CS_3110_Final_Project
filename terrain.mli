...
(* Contain all the t_stats a terrain has/needs. Info from JSON. *)
type t_stats = {name:string; atkBonus:int; defBonus:int; img: Sprite.image}

(* Terrain type tells which units can travel through it.*)
type terrain = Impassable of t_stats| Sea of t_stats | Plain of t_stats
              | Mountain of t_stats | City of t_stats | Forest of t_stats

(* [get_terrain class] returns an arbitrary terrain that of the class [class].
 * (i.e. get_terrain "grass" will get a grass terrain, whose t_stats are found
 * in the JSON)
 *
